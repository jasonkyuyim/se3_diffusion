    ...
    with torch.no_grad():
        ...
        for ix, protein in enumerate(dataset_valid):
            ...
            if args.score_only:
                ...
                # decode_file = base_folder + '/decode/' + batch_clones[0]['name'] + '.fa'
                decode_dir = os.path.join(base_folder, 'decode')
                os.makedirs(decode_dir)
                decode_file = os.path.join(decode_dir, batch_clones[0]['name'] + '.fa')
                ...
                np.savez(decode_file, decode_order=np.concatenate(all_decoding_orders))
                if args.save_score:
